...
export class LineCounter {
  ...
  private static readonly CODE_EXTENSIONS = new Set([
    ...
    '.vue', '.svelte', '.html', '.css', '.scss', '.sass', '.less', '.sql',
    '.xml', '.json', '.yaml', '.yml', '.toml', '.ini', '.conf', '.sh', '.bat',
    '.ps1', '.r', '.m', '.pl', '.lua', '.vim', '.dockerfile', '.makefile'
  ]);
  ...
  static countLinesInFile(filePath: string): FileLineInfo | null {
    try {
      ...
      logDebug(`Processing file: ${filePath}, extension: ${ext}, isCodeFile: ${isCodeFile}`);
      ...
      for (const line of lines) {
        ...
        if (inMultiLineComment) {
          commentLines++;
          if (this.COMMENT_PATTERNS.multiLineEnd.some(pattern => pattern.test(line))) {
        ...
        if (this.COMMENT_PATTERNS.multiLineStart.some(pattern => pattern.test(line))) {
          commentLines++;
        ...
        if (this.COMMENT_PATTERNS.singleLine.some(pattern => pattern.test(line))) {
          commentLines++;
          continue;
        }
      ...
      logDebug(`Code file ${filePath}: total=${result.lineCount.total}, code=${result.lineCount.code}, comments=${result.lineCount.comments}, blank=${result.lineCount.blank}`);
      return result;
