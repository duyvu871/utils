...
export class OutputManager {
  ...
  constructor(outDir?: string, countLines: boolean = false, config?: Config) {
    ...
    this.statisticsCollector = new StatisticsCollector(countLines, this.isConsoleOutput, finalConfig);
  ...
  async flush(rootDirName: string) {
    ...
    try {
      ...
      console.log(`📊 Report size: ${this.progressManager.formatBytes(Buffer.byteLength(content, 'utf8'))}`);
