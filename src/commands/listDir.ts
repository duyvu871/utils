...
function estimateItemCount(dirPath: string, config: Config): number {
  ...
  function countRecursive(currentPath: string, depth: number): void {
    ...
    try {
      ...
      const excludePatterns = (config['exclude-patterns'] || []).map((p: string) => new RegExp(p));
...
function listDirRecursive(
  ...
  try {
    ...
    outputManager.log(`${prefix}└── ❓ Error reading directory: ${path.basename(dirPath)}`);
    return;
  }
  ...
  const filteredItems = items.filter(item => {
    ...
    if (config.exclude && config.exclude.includes(item)) {
      logDebug(`Excluding '${item}' based on direct name match in 'exclude' list.`);
      return false;
    }
  ...
  filteredItems.forEach((item, index) => {
    ...
    try {
      ...
      if (stats.isDirectory()) {
        ...
        outputManager.updateProgress(progressTracker.current, progressTracker.total);
        
        // Recursive scan of subdirectory
        listDirRecursive(itemPath, newPrefix, config, rootScanPath, outputManager, progressTracker);
      } else {
 ...
 */
export async function executeListDir(options: { path?: string, config: Config }) {
  /**
   ...
   * ============================
   */
  const outputManager = new OutputManager(options.config.outDir, options.config.countLines, options.config);
  logDebug(`Output manager initialized with outDir: ${options.config.outDir || 'console'}, countLines: ${options.config.countLines}, topExtensions: ${options.config.topExtensions}`);
  ...
  try {
    ...
    if (!options.config.outDir) {
      ...
      listDirRecursive(rootPath, '', options.config, rootPath, outputManager, progressTracker);
    ...
    const terminalStats = outputManager.getStatisticsCollector().generateTerminalReport();
    ...
    console.log(`📁 Scanned: ${finalStats.totalDirectories.toLocaleString()} directories, ${finalStats.totalFiles.toLocaleString()} files`);
    console.log(`💾 Total size: ${outputManager.getProgressManager().formatBytes(finalStats.totalSize)}`);
    
    if (options.config.countLines && finalStats.totalLines.total > 0) {
      console.log(`📝 Code analysis: ${finalStats.totalLines.total.toLocaleString()} lines (${finalStats.codeFileCount} files)`);
    }
